...
int main(int argc, char *argv[])
{
  ...
  char   **OutFile;
  char **prefixlines;
  FILE *nsnapshots_file;
  char theline[MAXSTRING];
  /*==================================================================*
   *                          USER INTERFACE                          *
   *==================================================================*/
  ...
  if (argc < 2)
    {
      fprintf(stderr,"./MergerTree Num_snapshots \n");
      fprintf(stderr,"need a file called MTSnaps.txt that has the AHF prefix of every snapshot i.e. snap287RS.z1.030.AH");
      return 0;
    }
  nFiles = atoi(argv[1]);
  ...
  // scanf("%d", &nFiles);
  nsnapshots_file = fopen("MTSnaps.txt", "r");
  ...
  prefixlines  = (char **) calloc(nFiles, sizeof(char *));

  for(i=0; i<nFiles; i++)
   {
    HaloFile[i] = (char *) calloc(MAXSTRING, sizeof(char));
    OutFile[i]  = (char *) calloc(MAXSTRING, sizeof(char));
    prefixlines[i]  = (char *) calloc(MAXSTRING, sizeof(char));
   }
  
  /* read input filenames from stdin */
  for(i=0; i<nFiles; i++)
   {
     fscanf(nsnapshots_file, "%s", theline);
     sprintf(HaloFile[i], "%s_particles", theline);
     sprintf(prefixlines[i],"%s", theline);
     //    fprintf(stderr,"Please give name of %5d. *_particles file:            ",i+1);
     //    scanf("%s", HaloFile[i]);
    fprintf(stderr,"%s\n", HaloFile[i]);
   }
  fclose(nsnapshots_file);
  /* read output filenames from stdin */
  for(i=0; i<nFiles-1; i++)
     {
      fprintf(stderr,"Please give prefix for %5d. output file:                 ",i+1);
      //scanf("%s", OutFile[i]);
      //      sprintf(OutFile[i],"%s_mtree_idx.txt",prefixlines[i]);
      sprintf(OutFile[i],"%s",prefixlines[i]);           
      fprintf(stderr,"%s\n",OutFile[i]);
     }
  fprintf(stderr,"\n");
  ...
  for(i=0; i<nFiles-1; i++) {
    ...
    fprintf(stderr,"passed next block \n ");
    ...
    fprintf(stderr,"passed final block \n ");
...
int read_particles(char filename[MAXSTRING], int isimu)
{
      ...
      for(ipart=0; ipart<nPartInHalo; ipart++)
       {
        ...
        if(Ptype == ONLY_USE_PTYPE)
#endif
         {
          ...
          if(halos[isimu][ihalo].Pid[nPartInUse] > PidMax)       PidMax =       (halos[isimu][ihalo].Pid[nPartInUse]);
  ...
  fprintf(stderr," done in %ld sec. (full ID range = %"PRIu64" -> %"PRIu64", local ID range = %"PRIu64" -> %"PRIu64")\n",
          elapsed,PidMin,PidMax,PidMin_local,PidMax_local);
...
int particle_halo_mapping(int isimu)
{
  ...
  elapsed -= time(NULL);
  fprintf(stderr,"  o creating particle<->halo mapping for file %d (PidMax=%"PRIu64") ...",isimu,PidMax);
