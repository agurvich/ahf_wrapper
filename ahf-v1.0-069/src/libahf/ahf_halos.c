...
#endif
#ifdef WITH_OPENMP
#  pragma omp parallel for schedule (dynamic)	shared(halos, numHalos) private(i,k,numSubStruct,SubStruct,ihost,isub)
#endif
	for (i = 0; i < numHalos; i++) {
    ...
    for(k=0; k<halos[i].numSubStruct; k++)
     {
      ...
      ihost = halos[halos[i].subStruct[k]].hostHalo; // this should be identical to i !?
      ...
      if(halos[isub].hostHaloLevel >= AHF_HOSTHALOLEVEL)
       {
        if(check_subhalo(halos+ihost, halos+isub) == TRUE)
        ...
        else
         {
          ...
          halos[halos[i].subStruct[k]].hostHalo = -1;
         }
       }
